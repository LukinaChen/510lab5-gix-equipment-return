          ...
          {items.map((item) => (
            ...
            >
              {editingId === item.id ? (
                ...
                /* ── View mode ── */
                <div className="p-4">
                  {/* Name + edit — always full width */}
                  <div className="flex items-start justify-between gap-2 mb-3">
                    <p className={`font-medium text-base leading-snug flex-1 min-w-0 ${item.status === 'returned' ? 'line-through text-slate-400' : 'text-slate-800'}`}>
                      {item.name}
                    </p>
                    <button
                  ...
                  {(item.quantity > 1 || item.notes) && (
                    <div className="flex flex-wrap gap-3 mb-3">
                      ...
                      {item.notes && (
                        <span className="text-xs text-amber-600 italic">{item.notes}</span>
                      )}
                    </div>
                  )}

                  {/* Status buttons — always on their own row, wrap naturally */}
                  <div className="flex flex-wrap gap-2">
                    {item.status !== 'returned' && (
                      <button
                        onClick={() => updateStatus(item.id, 'returned')}
                        className="px-3 py-1.5 rounded-lg bg-emerald-100 text-emerald-700 text-xs font-semibold hover:bg-emerald-200 active:scale-95 transition-all"
                      >
                        ✓ Returned
                      </button>
                    )}
                    {item.status !== 'missing' && (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-red-100 text-red-600 text-xs font-semibold hover:bg-red-200 active:scale-95 transition-all"
                      >
                        ✗ Missing
                      </button>
                    )}
                    {item.status !== 'pending' && (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-slate-100 text-slate-500 text-xs font-semibold hover:bg-slate-200 active:scale-95 transition-all"
                      >
                        Reset
                      </button>
                    )}
